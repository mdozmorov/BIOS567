...
############## none
out <- topTable(fit3, adjust.method = "none", number = dim(exprs(methyl))[1], sort.by = "none")
sign <- rownames(out)[out$adj.P.Val < 0.05]
length(sign)

############### BH
out <- topTable(fit3, adjust.method = "BH", number = dim(exprs(methyl))[1], sort.by = "none")
sign <- rownames(out)[out$adj.P.Val < 0.05]
length(sign)

############### Holm
out <- topTable(fit3, adjust.method = "holm", number = dim(exprs(methyl))[1], sort.by = "none")
sign <- rownames(out)[out$adj.P.Val < 0.05]
length(sign)
